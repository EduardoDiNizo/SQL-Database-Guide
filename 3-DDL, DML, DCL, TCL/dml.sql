...
-- INNER JOIN para obter o nome do usuário e o nome da categoria do produto
SELECT u.name AS user_name, p.name AS product_name, pc.name AS category_name
FROM `order` o
...
-- LEFT JOIN para obter todos os usuários, mesmo os que não fizeram pedidos
SELECT u.name AS user_name, o.id AS order_id
FROM user u
LEFT JOIN `order` o ON u.id = o.user_id;

-- RIGHT JOIN para obter todos os pedidos, mesmo os que não têm um usuário associado
SELECT u.name AS user_name, o.id AS order_id
FROM `order` o
RIGHT JOIN user u ON o.user_id = u.id;

-- FULL JOIN para obter todos os usuários e todos os pedidos, combinando quando possível
SELECT u.name AS user_name, o.id AS order_id
FROM user u
